...
export default function AiProjectEditor({ project, onSave, onCancel, colors: c, isDark, existingNames }) {
  ...
  const handleSave = () => {
    ...
    // Check all file descriptions are filled
            ...
            {files.map((f, i) => (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                  <span style={{ fontSize: 11, fontFamily: 'monospace', color: c.chromeText, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis' }}>{f.path}</span>
                ...
                <input
                  ...
                  placeholder="Description (required) — what this file contains"
                  style={{
                    width: '100%',
                    background: 'transparent',
                    border: `1px solid ${!f.description.trim() ? '#DC2626' : inputBorder}`,
                    borderRadius: 4,
                    padding: '3px 6px',
                    fontSize: 11,
                    color: c.text,
